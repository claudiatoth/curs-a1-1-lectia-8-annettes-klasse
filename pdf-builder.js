...
(function () {
    ...
    function buildTheory() {
        ...
        return `<h1 class="chapter">📘 1. Teorie — Pluralbildung: cele 5 tipare de plural</h1>` + t;
    }
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += fillInBlock('Übung 1 — Articolul la plural este MEREU „die"', 'Indiferent de gen la singular (der/die/das), la plural articolul hotărât e mereu „die". Capcană: die Männer (m pl.) — articolul nu trădează genul.', ex1Data);
        }
        if (typeof ex2Data !== 'undefined') {
            html += fillInBlock('Übung 2 — Pluralul în -(e)n (feminine)', 'Aproape toate feminine: -e final → -n (Lampe → Lampen), -consoană → -en (Frau → Frauen).', ex2Data);
        }
        if (typeof ex3Data !== 'undefined') {
            html += fillInBlock('Übung 3 — Pluralul în -e (masculine + neutre)', 'Multe masculine cu vocală scurtă: Tisch → Tische, Hund → Hunde. Uneori cu Umlaut: Stuhl → Stühle.', ex3Data);
        }
        if (typeof ex4Data !== 'undefined') {
            html += fillInBlock('Übung 4 — Pluralul în -er (cu Umlaut frecvent)', 'Multe neutre + câteva masculine cu vocală a/o/u → ä/ö/ü: Buch → Bücher, Mann → Männer, Haus → Häuser.', ex4Data);
        }
        if (typeof ex5Data !== 'undefined') {
            html += fillInBlock('Übung 5 — Negația „kein" la plural + capcana -in → -innen', 'Plural: keine Bücher · keine Männer. Substantive feminine -in (Lehrerin) → plural -innen (Lehrerinnen — dublu n!).', ex5Data);
        }
        return html;
    }
